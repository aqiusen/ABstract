...
class FeatureFlagsImplTest {
    ...
    public void should_get_all_success() {
        when(repository.findAll()).thenReturn(Flux.just(createFeatureFlagDB("featureKey1"),
                createFeatureFlagDB("featureKey2"),
                createFeatureFlagDB("featureKey3")));
    ...
    public void should_get_success() {
        String featureKey = "featureKey1";
        when(repository.findById(anyString())).thenReturn(Mono.just(createFeatureFlagDB(featureKey)));
